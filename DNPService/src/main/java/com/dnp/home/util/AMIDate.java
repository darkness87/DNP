...
public class AMIDate extends Date implements Serializable {

	final static Logger logger = LogManager.getLogger(AMIDate.class);
	...
	public AMIDate(long l) {
		cal = new GregorianCalendar(timezone, locale);
		cal.setTime(new Date(l + (long)nHourOffset * 60 * 60 * 1000));
	}

	public AMIDate(int nYear, int nMonth, int nDate, int nHrs, int nMin, int nSec)
	{
		cal = new GregorianCalendar(timezone, locale);
		cal.set(1900+nYear, nMonth, nDate, nHrs+nHourOffset, nMin, nSec);
	}

	public AMIDate(int nYear, int nMonth, int nDate, int nHrs, int nMin) {
		cal = new GregorianCalendar(timezone, locale);
		cal.set(1900+nYear, nMonth, nDate, nHrs + nHourOffset, nMin);
	}

	public AMIDate(int nYear, int nMonth, int nDate) {
		cal = new GregorianCalendar(timezone, locale);
	 ...
	 * @throws Exception
	 */
	public AMIDate(String s) throws Exception
	{
		cal = new GregorianCalendar(timezone, locale);
	...
	public boolean after(AMIDate when) {
		return this.getTime() > when.getTime();
	}

	public boolean before(AMIDate when) {
		return this.getTime() < when.getTime();
	}

	public boolean equalDate(AMIDate when) {
	...
	public boolean equalDateTime(AMIDate when) {
		if (this.getYear() == when.getYear()
				&& this.getMonth() == when.getMonth()
				&& this.getDate() == when.getDate()
	 ...
	 * @DB
	 */
	public static AMIDate ParseDate(String szDate) throws Exception {
		int nBegin = 0;
		int nEnd = 0;
		int nYear = 0;
		int nMonth = 0;
		...
		AMIDate date = new AMIDate((nYear - 1900), (nMonth - 1), nDate);
		return date;
	}
	 ...
	 * @DB
	 */
	public static AMIDate ParseDateTime(String szDate) throws Exception {
		int nBegin = 0;
		int nEnd = 0;
		int nYear = 0;
		int nMonth = 0;
		...
		AMIDate date = new AMIDate((nYear-1900), (nMonth-1), nDate, nHour, nMin, nSec);
		return date;
	}
	 ...
	 * @DB
	 */
	public static AMIDate getHDDateValue(String szValue, int nFormat) {
		AMIDate hdDate = new AMIDate(1970-1900, 0, 1);
	 ...
	 * @DB
	 **/
	public static AMIDate getNewDateFormCurrentTime(int nDiffDay) {
		return getNewDate(new AMIDate(), nDiffDay, -1 , -1 , -1 ) ;
	}
	 ...
	 * @DB
	 **/
	public static AMIDate getNewDateFormCurrentTime(int nDiffDay, int nHour, int nMin, int nSec ) {
		return getNewDate(new AMIDate(), nDiffDay, nHour, nMin, nSec ) ;
	}
	 ...
	 * @DB
	 **/
	private static AMIDate getNewDate (AMIDate hDate, int nDiffDay, int nHour, int nMin, int nSec )
	{
		long lCurrentTime = hDate.getTime() + DAYMILISEC * nDiffDay ;
		hDate.setTime ( lCurrentTime );
		AMIDate hFindDate = null ;

		if ( nHour == -1 ) {
			hFindDate = new AMIDate(hDate.getYear(), hDate.getMonth(), hDate.getDate(),
					hDate.getHours(), hDate.getMinutes(), hDate.getSeconds());
		} else {
			hFindDate = new AMIDate(hDate.getYear(), hDate.getMonth(), hDate.getDate(), nHour , nMin , nSec);
		}
